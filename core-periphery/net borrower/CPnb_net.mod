...
l_AB b_AC b_AD b_BA l_CA l_DA
rb_AB rb_CA rb_DA
del_BA del_AC del_AD
...
@#include "liqinjvardefs_CPnb.mod" 
...
bprA_ss = (1/(1+rd_ss)-1)*d_ss+2*(1/(1+rb_ss)-del_ss)*b_ss
        +(alp_ss+dpf*(1-alp_ss)-1/(1+rl_ss))*x_ss
        + (del_ss+dpb*(1-del_ss)-1/(1+rb_ss))*l_ss
        -(omegab)*((1-del_ss)*b_ss)^2+Gamma_ss;

bprB_ss = (1/(1+rd_ss)-1)*d_ss+(1/(1+rb_ss)-del_ss)*b_ss
          +(alp_ss+dpf*(1-alp_ss)-1/(1+rl_ss))*x_ss
          -(omegab/2)*((1-del_ss)*b_ss)^2+Gamma_ss;

bprCD_ss = (1/(1+rd_ss)-1)*d_ss
          +(alp_ss+dpf*(1-alp_ss)-1/(1+rl_ss))*x_ss
...
svpCD_ss = (1/bprCD_ss)/(1-nu*(beta*del_ss+beta^2*dpb*(1-del_ss)-1/(1+rb_ss)))/(k*rwb*(1-beta*(1-xiCD)));
...
svp_A*b_AC(-1)  = beta*svp_A(+1)*omegab*(1-del_AC)*(b_AC(-1))^2 + dbA;                                                    // Equation (10): FOC wrt del_AC
svp_A*b_AD(-1)  = beta*svp_A(+1)*omegab*(1-del_AD)*(b_AD(-1))^2 + dbA;                                                    // Equation (10): FOC wrt del_AD

svp_A/(1+rb_AB) = beta*svp_A(+1)*del_BA(+1)+beta^2*svp_A(+2)*dpb*(1-del_BA(+1))-dFA*k*rwb*(del_ss/del_BA(+1))^etab;       // Equation (11): FOC wrt l_AB
...
bpr_A           = d_A/(1+rd_A)+b_AC/(1+rb_CA)+b_AD/(1+rb_DA)-l_AB/(1+rb_AB)-x_A/(1+rl_A)+del_BA*l_AB(-1)+alp_A*x_A(-1)
                    -del_AC*b_AC(-1)-del_AD*b_AD(-1)-d_A(-1)+dpb*(1-del_BA(-1))*l_AB(-2)+dpf*(1-alp_A(-1))*x_A(-2)
                    -omegab/2*(((1-del_AC(-1))*b_AC(-2))^2+((1-del_AD(-1))*b_AD(-2))^2)+Gamma_A*S_ss;                                                  // Equation (17): Bank A Profits
F_A             = (1-xiA)*F_A(-1)+nu*bpr_A;                                                                               // Equation (18): LoM of F_A

// B

svp_B*b_BA(-1)  = beta*svp_B(+1)*omegab*(1-del_BA)*(b_BA(-1))^2 + dbB;                                                    // Equation (19): FOC wrt del_BA
...
bpr_B           = d_B/(1+rd_B)+b_BA/(1+rb_AB)-x_B/(1+rl_B)+alp_B*x_B(-1)
                    -del_BA*b_BA(-1)-d_B(-1)+dpf*(1-alp_B(-1))*x_B(-2)
                    -omegab/2*((1-del_BA(-1))*b_BA(-2))^2+Gamma_B*S_ss;                                                  // Equation (26): Bank B Profits
...
bpr_C           = d_C/(1+rd_C)-l_CA/(1+rb_CA)-x_C/(1+rl_C)+del_AC*l_CA(-1)+alp_C*x_C(-1)
                    -d_C(-1)+dpb*(1-del_AC(-1))*l_CA(-2)+dpf*(1-alp_C(-1))*x_C(-2)
                    +Gamma_C*S_ss;                                                  // Equation (35): Bank A Profits                                                                       // Equation (): FOC wrt F_C
F_C             = (1-xiCD)*F_C(-1)+nu*bpr_C;                                                                               // Equation (36): LoM of F_C

// D

//svp_D*b_CA(-1)  = beta*svp_C(+1)*omegab*(1-del_CA)*(b_CA(-1))^2 + dbCD;                                                    // Equation (28): FOC wrt del_CA
svp_D/(1+rb_DA) = beta*svp_D(+1)*del_AD(+1)+beta^2*svp_D(+2)*dpb*(1-del_AD(+1))-dFCD*k*rwb*(del_ss/del_AD(+1))^etab;       // Equation (29): FOC wrt l_DA
...
bpr_D           = d_D/(1+rd_D)-l_DA/(1+rb_DA)-x_D/(1+rl_D)+del_AD*l_DA(-1)+alp_D*x_D(-1)
                    -d_D(-1)+dpb*(1-del_AD(-1))*l_DA(-2)+dpf*(1-alp_D(-1))*x_D(-2)
                    +Gamma_D*S_ss;                                                  // Equation (35): Bank A Profits                                                                       // Equation (): FOC wrt F_C
...
tot_l = l_AB+l_CA+l_DA;
tot_b = b_BA+b_AC+b_AD;
...
T_A*y_A+xiA*F_A(-1) = dpb*((1-del_AC(-1))*b_AC(-2)+(1-del_AD(-1))*b_AD(-2))+dpf*(1-alp_A(-1))*x_A(-2);
T_B*y_B+xiB*F_B(-1) = dpb*(1-del_BA(-1))*b_BA(-2)+dpf*(1-alp_B(-1))*x_B(-2);
T_C*y_C+xiCD*F_C(-1) = dpf*(1-alp_C(-1))*x_C(-2);
T_D*y_D+xiCD*F_D(-1) = dpf*(1-alp_D(-1))*x_D(-2);
...
rb_av = (rb_AB+rb_CA+rb_DA)/num_links;
...
IBspread_A1 = rb_AB - rb_CA;
IBspread_A2 = rb_AB - rb_DA;

@#include "liqinjvareqs_CPnb.mod"
...
del_BA = del_ss; del_AC = del_ss; del_AD = del_ss;
l_AB = l_ss; l_CA = l_ss; l_DA = l_ss;
b_AC = b_ss; b_AD = b_ss; b_BA = b_ss;      
...
rb_AB = rb_ss; rb_CA = rb_ss; rb_DA = rb_ss;
...
@#include "liqinjvarinit_CPnb.mod"
...
if strcmp(CPshockconfig,'SC1') && strcmp(liqinj,'off')
    ...
    irfs.SC1 = oo_.irfs;
    save('IRF_CPnb_noLI.mat','-struct','irfs','SC1');
    save('varsCPnb_noLI.mat','var_list_');

elseif strcmp(CPshockconfig,'SC1') &&  strcmp(liqinj,'on')

    irfs.SC1 = oo_.irfs;
    save('IRF_CPnb_LI.mat','-struct','irfs','SC1');
    save('varsCPnb_LI.mat','var_list_');

elseif strcmp(CPshockconfig,'SC2') &&  strcmp(liqinj,'off')

    irfs.SC2 = oo_.irfs
    save('IRF_CPnb_noLI.mat','-struct','irfs','SC2','-append');

elseif strcmp(CPshockconfig,'SC2') &&  strcmp(liqinj,'on')

    irfs.SC2 = oo_.irfs
    save('IRF_CPnb_LI.mat','-struct','irfs','SC2','-append');

elseif strcmp(CPshockconfig,'SC3') &&  strcmp(liqinj,'off')

    irfs.SC3 = oo_.irfs
    save('IRF_CPnb_noLI.mat','-struct','irfs','SC3','-append');

elseif strcmp(CPshockconfig,'SC3') &&  strcmp(liqinj,'on')

    irfs.SC3 = oo_.irfs
    save('IRF_CPnb_LI.mat','-struct','irfs','SC3','-append');

elseif strcmp(CPshockconfig,'SC4') &&  strcmp(liqinj,'off')

    irfs.SC4 = oo_.irfs
    save('IRF_CPnb_noLI.mat','-struct','irfs','SC4','-append');

elseif strcmp(CPshockconfig,'SC4') &&  strcmp(liqinj,'on')

    irfs.SC4 = oo_.irfs
    save('IRF_CPnb_LI.mat','-struct','irfs','SC4','-append');
